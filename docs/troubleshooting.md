...
### Top 5 False Positive Scenarios

False positives are warnings emitted by the analyzer that do not represent real
performance issues in your workload. The sections below describe the most common
cases and how to silence them.

---

#### 1. N+1 warnings on ORM-generated queries (Entity Framework, Dapper)

**Symptom**: The N+1 detector fires on a group of queries issued by EF Core or
Dapper even though each query targets a different row by primary key.

**Why it happens**: The detector counts how many times the same table appears
across a query batch. ORMs often issue many single-row look-ups in sequence,
which looks like N+1 from a structural standpoint even when they are backed by
an efficient index seek.

**Solutions**:

1. Use `IgnorePatterns` to suppress queries whose text matches a pattern:
...
2. Alternatively, disable N+1 detection globally and rely on EF Core's own
   query logging:

```json
{ "Analysis": { "DetectNPlusOne": false } }
```

---

#### 2. N+1 warnings on CTEs that reference the same table multiple times

**Symptom**: A query using `WITH cte AS (SELECT * FROM orders) SELECT * FROM cte JOIN cte c2 ON ...`
triggers a spurious N+1 warning.

**Why it happens** (fixed in v1.1): In earlier versions, CTE alias names
(`cte`) were treated as physical table names and inflated the hit-count.
This is now fixed — CTE aliases are excluded from table-reference counting.

**If you see this on an older version**, upgrade to v1.1 or add the CTE alias
to `IgnorePatterns`:

```json
{ "Analysis": { "IgnorePatterns": ["FROM cte"] } }
```

---

#### 3. Explain plan parser does not recognize vendor-specific query hints

**Symptom**: `OPTION (RECOMPILE)`, `FORCE INDEX`, `/*+ INDEX(...) */` or similar
hints cause the plan parser to emit unexpected warnings or fail to parse the plan.

**Solutions**:

- Strip hints before passing the query text to the analyzer:

```csharp
var cleanQuery = Regex.Replace(rawQuery, @"/\*\+.*?\*/", string.Empty);
```

- Report unrecognized hint syntax on GitHub so it can be added to the parser.

---

#### 4. Missing-index warnings on small lookup tables

**Symptom**: A `MissingIndex` warning fires on a table with only a few hundred
rows where a full scan is genuinely cheaper than an index seek.

**Why it happens**: The default severity thresholds treat all missing indexes
the same regardless of table size.

**Solution** — tune the thresholds so small-table warnings are downgraded to
`Info` level:
...
With `InfoMaxRows: 10000`, any missing-index issue on a table with ≤ 10 000
estimated rows will be reported as `Info` rather than `Warning` or `Critical`.
Tune these values to match your environment:

| Database size       | Recommended InfoMaxRows | WarningMaxRows |
|---------------------|------------------------|----------------|
| Small (dev/test)    | 50 000                 | 500 000        |
| Medium (production) | 10 000 (default)       | 1 000 000      |
| Large (OLAP/DW)     | 1 000                  | 10 000 000     |

---

#### 5. SELECT * warnings inside views or generated queries

**Symptom**: `SelectStar` warnings appear on queries you cannot change (e.g.,
auto-generated by an ORM or inside a database view).

**Solution** — add a pattern to `IgnorePatterns`:

```json
{ "Analysis": { "IgnorePatterns": ["FROM vw_", "FROM \\[dbo\\]\\[View"] } }
```
...
If none of the above applies, please open a GitHub issue and include:

1. The exact SQL query that triggers the false positive (sanitize sensitive column names).
2. The analyzer version (`dotnet list package | grep SqlQueryAnalyzer`).
3. The database type and version (SQL Server, PostgreSQL, MySQL…).
4. The full analyzer output with `ANALYZER_LOG_LEVEL=Debug`.
5. Why you believe the warning is incorrect.
