...
public static class QueryCacheKeyGeneratorExtensions
{
    ...
    public static Dictionary<string, string>? ExtractParametersFromMetadataKey(this QueryCacheKeyGenerator generator, string metadataKey)
    {
        ...
        // The key only carries a SHA256 hash of the normalized query and parameters, which is a
        // one-way transformation. The original parameter names and values cannot be recovered
        // from it, so returning null here is correct, not a placeholder for missing behavior.
        return null;
    }
    ...
    /// <returns>
    /// True if the key is invalid, was not generated by this process, or its age exceeds
    /// <paramref name="maxAgeHours"/>; false otherwise.
    /// </returns>
    ...
    public static bool IsCacheKeyExpired(this QueryCacheKeyGenerator generator, string key, int maxAgeHours)
    {
        ...
        // Key age is tracked in-process at generation time (see QueryCacheKeyGenerator.GetKeyCreatedAt).
        // Keys generated by a previous process run have no recorded timestamp and are treated as
        // expired, since their real age cannot be determined.
        var createdAt = generator.GetKeyCreatedAt(key);
        if (createdAt == null)
            return true;

        return DateTimeOffset.UtcNow - createdAt.Value > TimeSpan.FromHours(maxAgeHours);
    }
