...
The `QueryCacheKeyGenerator` class generates deterministic cache keys for SQL queries and analysis results. It ensures consistent key generation across the application by using SHA256 hashing of normalized query text. The generator supports different key types (query, result, metadata, batch) and provides validation, extraction, and type identification methods for working with cache keys.

### Usage Example

```csharp
// Initialize the cache key generator
var keyGenerator = new QueryCacheKeyGenerator();

// Generate a cache key for a SQL query
var query = "SELECT * FROM Users WHERE Status = 'active'";
var queryKey = keyGenerator.GenerateQueryKey(query);
Console.WriteLine($"Query cache key: {queryKey}");

// Generate a cache key for analysis results
var resultKey = keyGenerator.GenerateResultKey(query);
Console.WriteLine($"Result cache key: {resultKey}");

// Generate a cache key for analysis metadata with parameters
var metadataKey = keyGenerator.GenerateMetadataKey(query, new Dictionary<string, string> 
{
    {"timeout", "30"},
    {"includeExecutionPlan", "true"}
});
Console.WriteLine($"Metadata cache key: {metadataKey}");
...
var queries = new string[]
{
    ...
    "SELECT u.Name, COUNT(o.Id) FROM Users u JOIN Orders o ON u.Id = o.UserId GROUP BY u.Name"
};
var batchKey = keyGenerator.GenerateBatchKey(queries);
Console.WriteLine($"Batch cache key: {batchKey}");

// Validate a cache key
var isValid = keyGenerator.IsValidCacheKey(queryKey);
Console.WriteLine($"Is valid cache key: {isValid}");

// Extract hash from a cache key
var hash = keyGenerator.ExtractHashFromKey(queryKey);
Console.WriteLine($"Extracted hash: {hash}");

// Get cache key type
var keyType = keyGenerator.GetKeyType(queryKey);
Console.WriteLine($"Key type: {keyType}");

// Check when a key was created
var createdAt = keyGenerator.GetKeyCreatedAt(queryKey);
if (createdAt.HasValue)
{
    Console.WriteLine($"Key created at: {createdAt.Value:yyyy-MM-dd HH:mm:ss}");
}
```

### Public Members

- `GenerateQueryKey(string query)` - Generates a cache key for a SQL query using SHA256 hash of normalized query text
- `GenerateResultKey(string query)` - Generates a cache key for analysis results, linking it to the query key
- `GenerateMetadataKey(string query, Dictionary<string, string>? parameters = null)` - Generates a cache key for analysis metadata with optional parameters
- `GenerateBatchKey(string[] queries)` - Generates a batch cache key for multiple queries
- `IsValidCacheKey(string key)` - Validates if a key was generated by this generator and has proper format
- `ExtractHashFromKey(string key)` - Extracts the query hash from a cache key for debugging and statistics
- `GetKeyType(string key)` - Gets the type of cache key (Query, Result, Metadata, Batch, or Unknown)
- `GetKeyCreatedAt(string key)` - Gets the timestamp when a key was generated (returns null for keys from other processes)
